...
const { data: surround } = await useAsyncData(`wiki-surround-${route.path}`, () =>
  queryCollectionItemSurroundings('wiki', route.path, { fields: ['title', 'description'] }), { default: () => [] })
    ...
    <UPage>
      ...
      <UPageBody>
        ...
        <p class="mt-8 text-sm text-[var(--ui-text-dimmed)]">
          <ULink :to="editUrl" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 hover:text-gold-400">
      ...
      <template #right>
        <UPageAside>
          <UContentToc v-if="page.body?.toc?.links?.length" :links="page.body.toc.links" highlight />
        </UPageAside>
      </template>
    </UPage>
  </UContainer>
